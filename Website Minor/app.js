...
app.get("/new", async (req, res) => {
  ...
  res.render("new.ejs", {
    headsCount: headsCount.rows[0].head,
    tailsCount: tailsCount.rows[0].tail,
    listItems: listItems,
  });
...
app.post("/flip10", async (req, res) => {
  for (let i = 0; i < 10; i++) {
    ...
    if (result === "Heads") {
      headsCount = await db.query("SELECT head FROM count");
      await db.query(
        "UPDATE count SET head = " + (headsCount.rows[0].head + 1)
      );
      listItems.push("Heads");
    } else if (result === "Tails") {
      tailsCount = await db.query("SELECT tail FROM count");
      await db.query(
        "UPDATE count SET tail = " + (tailsCount.rows[0].tail + 1)
      );
      listItems.push("Tails");
    }
  }
